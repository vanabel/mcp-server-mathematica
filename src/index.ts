...
const PROTECTED_SYMBOL_RENAMES: Record<string, string> = {
  D: "Dval",
  I: "Ival",
  E: "Eval",
  N: "Nval",
...
type MathematicaPreprocessResult = {
  code: string;
  rewrites: string[];
  blockedByRecursion: string[];
};

type NormalizeMathInputResult = {
  normalizedText: string;
  warnings: string[];
  detectedIssues: string[];
};

type AnalyzeMathStepResult = {
  kind: "instruction" | "equation" | "expression" | "unknown";
  originalText: string;
  normalizedText: string;
  parseable: boolean;
  warnings: string[];
...
type DifferentiateSymbolicResult = {
  normalizedExpression: string;
  normalizedVariable: string;
  normalizedAssumptions: string;
  derivative: string;
};
...
let promptTemplatesCache: Record<string, PromptTemplateDefinition> | null = null;
let referenceResourcesCache: ReferenceResource[] | null = null;

async function loadPromptTemplates(): Promise<Record<string, PromptTemplateDefinition>> {
  ...
  const templates = parsed as Record<string, PromptTemplateDefinition>;
...
async function loadReferenceResources(): Promise<ReferenceResource[]> {
  ...
  for (const entry of parsed) {
    ...
    const resource = entry as ReferenceResource;
    if (
  ...
  referenceResourcesCache = parsed as ReferenceResource[];
  return referenceResourcesCache;
}

async function reloadPromptAndResourceConfigs(): Promise<{ promptCount: number; resourceCount: number }> {
...
async function diagnoseMcpCapabilities(): Promise<Record<string, unknown>> {
...
async function renderPromptTemplate(templateName: string, params: Record<string, string>): Promise<string> {
  const templates = await loadPromptTemplates();
  ...
  for (const requiredParam of template.requiredParams) {
    if (!params[requiredParam]) {
      throw new McpError(
        ...
        `Missing required template param: ${requiredParam}`
      );
    }
  }
...
const formatToolError = (code: string, summary: string, hint: string, details?: string): string => {
  return [
    `ErrorCode: ${code}`,
    `Summary: ${summary}`,
    `Hint: ${hint}`,
...
function preprocessMathematicaCode(rawCode: string): MathematicaPreprocessResult {
  let code = rawCode.normalize("NFC");
  const rewrites: string[] = [];
  const blockedByRecursion: string[] = [];
  const codeForChecks = code.replace(/\(\*[\s\S]*?\*\)/g, " ");

  const symbolNormalizationRules: Array<[RegExp, string, string]> = [
...
function normalizeMathInput(text: string, mode: string = "mixed"): NormalizeMathInputResult {
...
function isInstructionText(text: string, instructionKeywords: string[] = []): boolean {
  const trimmed = text.trim();
  const lower = trimmed.toLowerCase();
  const builtInMatch =
    /^(given|also given|assume|let|find|compute|show|prove|derive|verify|we want to find|step\s*\d+:)/.test(lower) ||
...
function classifyStepKind(text: string, instructionKeywords: string[] = []): AnalyzeMathStepResult["kind"] {
  const trimmed = text.trim();
...
async function analyzeMathStep(
  text: string,
  mode: string = "mixed",
  instructionKeywords: string[] = []
): Promise<AnalyzeMathStepResult> {
...
async function differentiateSymbolic(
  expression: string,
  variable: string,
  assumptions: string = "True",
  format: string = "text",
  mode: string = "mixed"
): Promise<DifferentiateSymbolicResult> {
...
function classifyExecutionOutput(output: string): { code: string; summary: string; hint: string } | null {
...
async function verifyDerivation(
  ...
  format: string = "text",
  instructionKeywords: string[] = [],
  debugNormalizedSteps: boolean = false
): Promise<string> {
  if (steps.length < 2) {
    throw new McpError(
      ErrorCode.InvalidParams,
  ...
  try {
    ...
    const analyzedSteps = await Promise.all(
      steps.map((step) => analyzeMathStep(step, "mixed", instructionKeywords))
    );
  ...
  switch (request.params.name) {
      ...
      try {
          ...
          return {
            content: [{
              ...
              text: formatToolError(
                "PrecheckRecursiveAssignment",
                "Detected self-referential assignment that is likely to trigger recursion overflow.",
                "Rename left-hand variables to non-function names (for example rSym = r[alpha, tau]).",
                `Symbols: ${Array.from(new Set(processed.blockedByRecursion)).join(", ")}`
              )
            }],
            isError: true
          };
        ...
        return {
          content: [{
            ...
            text: formatToolError(
              "ExecutionFailed",
              "Failed to execute Mathematica code.",
              "Check Mathematica syntax and use non-protected symbol names.",
              error.message
            )
          }],
          isError: true
    ...
    case "verify_derivation": {
      ...
      const instructionKeywords = Array.isArray(request.params.arguments?.instructionKeywords)
        ? (request.params.arguments?.instructionKeywords as unknown[])
        ...
        return {
          content: [{
            ...
            text: formatToolError(
              ...
              "Failed to verify derivation steps.",
              "Ensure each symbolic step is valid Mathematica input. For natural language steps, rely on built-in instruction detection or provide instructionKeywords.",
              error.message
            )
          }],
          isError: true
    ...
    case "differentiate_symbolic": {
      ...
      if (!expression || !variable) {
        throw new McpError(
          ErrorCode.InvalidParams,
          "Both expression and variable are required"
        );
      }
      ...
      try {
        ...
        if (differentiated.derivative.trim() === "ParseError") {
          return {
            content: [{
              type: "text",
              text: formatToolError(
                "DifferentiationParseError",
                "Failed to parse expression, variable, or assumptions for symbolic differentiation.",
                "Use Mathematica-compatible syntax or run normalize_math_input first.",
                JSON.stringify(differentiated, null, 2)
              )
            }],
            isError: true
          };
        ...
        return {
          content: [{
            type: "text",
            text: formatToolError(
              "SymbolicDifferentiationFailed",
              "Failed to differentiate expression symbolically.",
              "Check expression/variable syntax and assumptions.",
              error.message
            )
          }],
          isError: true
      ...
      try {
        ...
        return {
          content: [{
            type: "text",
            text: formatToolError(
              "ReloadPromptResourcesFailed",
              "Failed to reload prompt/resource configuration files.",
              "Check config/prompts.json and config/resources.json for valid JSON and schema.",
